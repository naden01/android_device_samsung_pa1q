...
# <TOP>/device/samsung/pa2q/vendorsetup.sh, so this script is three dirs deep + /patches.
_SELF="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
if [ -n "$ANDROID_BUILD_TOP" ] && [ -d "$ANDROID_BUILD_TOP/bootable/recovery" ]; then
    TOP="$ANDROID_BUILD_TOP"
else
    # patches dir is <TOP>/device/samsung/pa2q/patches -> go up 4
    TOP="$(cd "$_SELF/../../../.." && pwd)"
fi
...
for patch in "$_SELF"/*.patch; do
    ...
    case "$name" in
        ...
        0019-fix-data-storage-path.patch)  marker="fixstoragepath:gui/gui.cpp" ;;
        *)                              marker="" ;;
    esac
    if [ -n "$marker" ]; then
